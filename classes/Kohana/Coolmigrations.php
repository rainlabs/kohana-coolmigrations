 ...
 *
 * @package		Coolmigrations
 * @author      Vladimir Zyablitskiy
 * @author 		Matías Montes
 * @author 		Jamie Madill
 * @author    	Fernando Petrelli
 */
class Kohana_Coolmigrations {

    protected $_config;

    public function __construct() {
        $this->_config = Kohana::$config->load('coolmigrations')->as_array();
    }
    ...
    protected function load_migration($version, $group = 'default') {
        ...
        $change = false;
        if ((!$this->method_defined($class, 'up') OR !$this->method_defined($class, 'down')) && !$this->method_defined($class, 'change'))
            throw new Kohana_Exception('Up/down/change functions missing on class :class', array(':class' => $class));
        else if (($this->method_defined($class, 'up') OR $this->method_defined($class, 'down')) && $this->method_defined($class, 'change'))
            throw new Kohana_Exception('Up/down functions with change function not resolve :class', array(':class' => $class));
        if ($this->method_defined($class, 'change'))
            $change = true;

        return new $class($group, $change);
    }
